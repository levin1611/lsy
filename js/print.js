...
forSPrint.data ={
	artiBodyId 	: "newContent",				//页面内主要内容 node id	
	titleId    	: "newContentTitle",			//页面标题部分node id	
	pageLogoId 	: "pagelogo",	//页面logo容器的id
	pageWayId	: "lo_links",	//页面路点容器的id

	artSourceId : "art_source",	//art_source容器的Id
	pubDateId   : "pub_date",	//发布时间容器的Id
	mediaNameId : "media_name",	//媒体来源容器的Id
	...
	artSourceHtml:"",	//初始显示的art_source
	pubDateHtml : "",	//初始显示的发布时间html
	mediaNameHtml:"",	//初始显示的媒体来源html
...
forSPrint.GetPageInitInfo = function() {
	...
	if(pageNode){
		...
		var html = pageNode.innerHTML;
		if((/(<img)+/ig).test(html)){//for img page 带有img标签的分页
		...
		}else if((/<\/p>/gi).test(html)){//for text page 2008-06之前的分页版本
		/*
		<span id="_function_code_page"><p align=right><a style="FONT-SIZE: 14px" href="page1.html">[上一页]</a>　<a href="page1.html">[1]</a>　[2]　<a href="page3.html">[3]</a>　<a href="page4.html">[4]</a>　<a href="page5.html">[5]</a>　<a style="FONT-SIZE: 14px" href="page3.html">[下一页]</a></p></span><!--page_info=5_2_1345982-->
		*/
		...
		}else{//2008-06之后的新分页版本
		/*
			<div class='pagebox' id='_function_code_page'>

<span class='pagebox_pre'><a href='http://news.sina.com.cn/c/p/2008-05-26/174015621790.shtml'>上一页</a></span>
<span class='pagebox_num'><a href='http://news.sina.com.cn/c/p/2008-05-26/174015621786.shtml'>1</a></span>
<span class='pagebox_num_nonce'>2</span>
<span class='pagebox_num'><a href='http://news.sina.com.cn/c/p/2008-05-26/174015621792.shtml'>3</a></span>
<span class='pagebox_next'><a href='http://news.sina.com.cn/c/p/2008-05-26/174015621792.shtml'>下一页</a></span>

</div></td></tr></table></div><!--page_info=3_2_15621786-->
		*/
			//hrefPattern = /(<a href=[\"\'].*?[\"\']>){0,1}\d+(<\/a>){0,1}/ig;
			hrefPattern = /<span[^>]*>\s*(<a\s+href=[\"\'][^>]*[\"\']>)?[0-9]+(<\/a>)?<\/span>/gi;
   ...
   //获取初始显示的   logo html
	...
	//获取初始显示的   art_source html
	if(forSPrint.data.artSourceId != '' && document.getElementById(forSPrint.data.artSourceId) != null)
	{
		forSPrint.data.artSourceHtml = document.getElementById(forSPrint.data.artSourceId).innerHTML;
	}

	//获取初始显示的发布时间html
	if(forSPrint.data.pubDateId != '' && document.getElementById(forSPrint.data.pubDateId) != null)
	{
		forSPrint.data.pubDateHtml = document.getElementById(forSPrint.data.pubDateId).innerHTML;
	}

	//获取初始显示的媒体来源html
	if(forSPrint.data.mediaNameId != '' && document.getElementById(forSPrint.data.mediaNameId) != null)
	{
		forSPrint.data.mediaNameHtml = document.getElementById(forSPrint.data.mediaNameId).innerHTML;
	}

	//090525001 ws begin
	//获取播放器代码
	if(document.getElementById("p_player")){
		//alert(FlvPlayerCode.data.playerCode);
		forSPrint.data.playerCode = FlvPlayerCode.data.playerCode;
	}
	//090525001 ws end
...
forSPrint.Print = function(){
	...
	var info =	"<html>"	+"\n"+
		...
		".pb,.otherContent_01,.sv_cont_c14{display:none;}" + "\n"+
		...
		"	artSourceId : \""+forSPrint.data.artSourceId+"\",	//art_source容器的Id"	+"\n"+
		"	pubDateId   : \""+forSPrint.data.pubDateId+"\",		//发布时间容器的Id"	+"\n"+
		"	mediaNameId : \""+forSPrint.data.artSourceId+"\",	//媒体来源容器的Id"	+"\n"+
		"	"	+"\n"+
		...
		"	artSourceHtml:\""+escape(forSPrint.data.artSourceHtml)+"\",	//初始显示的art_source"	+"\n"+
		"	pubDateHtml : \""+escape(forSPrint.data.pubDateHtml)+"\",	//初始显示的发布时间html"	+"\n"+
		"	mediaNameHtml:\""+escape(forSPrint.data.mediaNameHtml)+"\",	//初始显示的媒体来源html"	+"\n"+
		"	"	+"\n"+
		...
		"          </tr>"	+"\n"+
		"        </table></td>"	+"\n"+
		"      <td width=\"90\"><a href=\"javascript:sPrint.Print();\"><img src=\"http://image2.sina.com.cn/bb/article/con_pri_ws_001.gif\" width=\"80\" height=\"20\" alt=\"打印此页面\"/></a></td>"	+"\n"+
		"      <td width=\"65\"><a href=\"###\" id=\"btnExit1\"><img src=\"http://image2.sina.com.cn/bb/article/con_pri_ws_002.gif\" width=\"64\" height=\"20\" alt=\"返回正文页\"/></a></td>"	+"\n"+
		"    </tr>"	+"\n"+
		"  </table>"	+"\n"+
		...
		"        <table cellspacing=\"0\" width=\"560\">"	+"\n"+
		"          <tr align=\"center\">"	+"\n"+
		"            <td style=\"font-size:12px; font-family:Arial,Helvetica,sans-serif; padding:10px 0 15px; text-align:center; line-height:14px;\" id=\"artInfo\"></td>"	+"\n"+
		"          </tr>"	+"\n"+
		"        </table>"	+"\n"+
		"        <div id=\"PriCon\"></div>"	+"\n"+
		...
		"        </table></td>"	+"\n"+
		"      <td width=\"90\"><a href=\"javascript:sPrint.Print();\"><img src=\"http://image2.sina.com.cn/bb/article/con_pri_ws_001.gif\" width=\"80\" height=\"20\" alt=\"打印此页面\"/></a></td>"	+"\n"+
		"      <td width=\"65\"><a href=\"###\" id=\"btnExit2\"><img src=\"http://image2.sina.com.cn/bb/article/con_pri_ws_002.gif\" width=\"64\" height=\"20\" alt=\"返回正文页\"/></a></td>"	+"\n"+
		"    </tr>"	+"\n"+
		"  </table>"	+"\n"+
		"</center>"	+"\n"+
		"<!-- print dialog start -->"	+"\n"+
		...
		"sPrint.artInfoNode= sPrint.$(\"artInfo\"); 	//文章标题 node"	+"\n"+
		...
		//090525001 ws begin
		//每次载入分页数据时，都将播放器干掉
		" if(document.getElementById(\"p_player\")){document.getElementById(\"p_player\").innerHTML=\"<img src='http://i0.sinaimg.cn/dy/news_test/dtloading.gif' style='border:none' \/>\"}"	+"\n"+
		//090525001 ws end
		"	//输出加载完毕信息"	+"\n"+
		...
		"			if(document.getElementById(\"p_player\")){"+"\n"+
		"				document.getElementById(\"p_player\").innerHTML = '" + forSPrint.data.playerCode + "';"+"\n"+
		"			}"+"\n"+
		"		};"+"\n"+
		"		getFlvPlayerCode();"+"\n"+
		//090525001 ws end
		...
		"	sPrint.artInfoNode.innerHTML = unescape(sPrint.data.artSourceHtml) + \"&nbsp;&nbsp;\" + unescape(sPrint.data.pubDateHtml) + \"&nbsp;&nbsp;\" + unescape(sPrint.data.mediaNameHtml);	//显示页面信息"	+"\n"+
		...
		"	}"	+"\n"+
		"}"	+"\n"+
		"document.getElementById(\"btnExit1\").href=location.href;" + "\n" +
		"document.getElementById(\"btnExit2\").href=location.href;" + "\n" +
